    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/agency/staff-performance" className="text-primary-400 hover:text-primary-300 font-medium">
            Back to Performance
          </Link>
        </div>
      </div>
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="dark-card text-center">
          <Settings className="w-16 h-16 text-qs-muted mx-auto mb-4" />
          <h2 className="text-xl font-semibold text-qs-bright mb-2">
            Set up compensation model for {producerName}
          </h2>
          <p className="text-qs-dim mb-6 max-w-md mx-auto">
  ...
  return (
    <div className="dark-page max-w-7xl mx-auto px-4 py-6">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-2xl font-bold text-qs-bright">
            {producerName} — Compensation Model
          </h1>
          {carrier && (
            <p className="text-sm text-qs-subtle mt-0.5">
      ...
      {/* Mode toggle */}
      <div className="flex items-center gap-1 mb-6 bg-qs-elevated rounded-lg p-1 w-fit">
        <button
          ...
          className={`inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-md transition-colors ${
            mode === 'projection'
              ? 'bg-qs-card text-qs-bright'
              : 'text-qs-subtle hover:text-qs-text'
          }`}
        >
      ...
      {mode === 'projection' && (
        <>
          {/* Projection sub-tabs */}
          <div className="flex gap-1 mb-6 border-b border-qs-border">
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-2 px-4 py-2.5 text-sm font-medium border-b-2 transition-colors -mb-px ${
                    isActive
                      ...
                      : 'border-transparent text-qs-subtle hover:text-qs-text hover:border-qs-border'
                  }`}
                >
      ...
      {mode === 'actuals' && (
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            <label className="text-sm font-medium text-qs-text">Month:</label>
            <input
              type="month"
              value={selectedMonth}
              onChange={(e) => setSelectedMonth(e.target.value)}
              className="dark-input border border-qs-border rounded-lg px-3 py-1.5 text-sm"
            />
          </div>
          ...
          {loadingActuals ? (
            <div className="flex items-center justify-center py-12">
              <div className="text-center">
                ...
                <p className="text-sm text-qs-subtle">Loading actuals for {monthLabel}...</p>
              </div>
            </div>
          ) : (
            <>
