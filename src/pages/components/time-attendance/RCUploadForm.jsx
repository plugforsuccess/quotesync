...
export default function RCUploadForm({ orgId, weekStart, employeeMap, onUploaded }) {
  ...
  async function doUpload(matchedRows, skippedCount) {
    ...
    if (error) {
      ...
      setMsg({ type: 'success', text: `${records.length} records uploaded${skipNote}. Running alert scan...` });
      setPreview(null);
      setWarnings([]);
      if (fileRef.current) fileRef.current.value = '';

      // Await discrepancy detection and report results
      try {
        const { data: detectResult, error: detectError } = await supabase.functions.invoke(
        ...
        const alertInfo = detectResult?.new_alerts > 0
          ? `${detectResult.new_alerts} alert${detectResult.new_alerts !== 1 ? 's' : ''} found.`
          : 'No new alerts.';
        setMsg({ type: 'success', text: `${records.length} records uploaded${skipNote}. ${alertInfo}` });
      } catch (err) {
        console.error('[RC_UPLOAD] Discrepancy detection failed:', err);
        setMsg({
          type: 'warning',
          text: `${records.length} records uploaded${skipNote}. Alert scan failed — alerts will be generated by the nightly job.`,
        });
      }
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-6">
      ...
      {msg && (
        <div className={`mt-3 flex items-center gap-1.5 text-sm ${
          msg.type === 'error' ? 'text-red-600' : msg.type === 'warning' ? 'text-yellow-600' : 'text-green-600'
        }`}>
          {msg.type === 'error' ? <AlertCircle className="w-4 h-4" /> : msg.type === 'warning' ? <AlertCircle className="w-4 h-4" /> : <CheckCircle className="w-4 h-4" />}
          {msg.text}
        </div>
      )}
