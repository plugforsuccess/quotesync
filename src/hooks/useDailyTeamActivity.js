    ...
    queryFn: async () => {
      ...
      const [
        ...
        supabase.from('pending_cancel_attempts')
          .select('employee_id, result, auto_logged, pending_case_id, pending_cases(customer_name)')
          .eq('agency_id', agencyId).gte('attempted_at', startIso).lt('attempted_at', endIso),
        supabase.from('renewal_attempts')
          .select('employee_id, result, auto_logged, renewal_case_id, renewal_cases(customer_name)')
          .eq('agency_id', agencyId).gte('attempted_at', startIso).lt('attempted_at', endIso),
      ...
      const rep = (id) => (byRep[id] ||= {
        attempts: 0, reached: 0, cancelSaves: 0, renewalSaves: 0, premium: 0, tasksDone: 0, worked: {},
      });
      const touch = (r, kind, caseId, name, result) => {
        const key = `${kind}:${caseId}`;
        const w = (r.worked[key] ||= { name, kind, result: null, saved: false, premium: 0 });
      ...
      for (const a of cancelAtt || []) {
        ...
        touch(r, 'cancel', a.pending_case_id, a.pending_cases?.customer_name, a.result);
      }
      for (const a of renewalAtt || []) {
        if (a.auto_logged || !a.employee_id) continue;
        const r = rep(a.employee_id); r.attempts++; if (a.result === 'reached') r.reached++;
        touch(r, 'renewal', a.renewal_case_id, a.renewal_cases?.customer_name, a.result);
      }
