-- Referral Rewards v4 — frictionless self-report attribution
-- Adds the zero-enrollment capture path: the referred person names their
-- referrer at quote time (or via the AI phone intake). The materializer
-- find-or-creates that referrer. Also: a self-referral guard, and the
-- trigger-coverage fix from the pipeline audit (materialize when a
-- lead_quotes row becomes 'enriched' via UPDATE, not only on INSERT —
-- a failed-then-retried enrichment was silently dropping referrals).
...
BEGIN
  ...
  -- Resolve referrer, in priority order:
  --   1. explicit call-in link (referred_by_referrer_id)
  --   2. share-link code (referral_code)
  --   3. self-reported name/phone (find-or-create) — the zero-friction path
  ...
  IF v_ref IS NULL
    ...
    IF v_ref IS NULL THEN
      INSERT INTO public.referral_referrers (agency_id, name, phone)
      VALUES (v_lead.agency_id, btrim(v_lead.referred_by_name), v_ph)
  ...
  -- Self-referral guard: a lead cannot be its own referrer.
...
-- Trigger-coverage fix: also materialize when an existing lead_quotes row
-- transitions to 'enriched' (the failed-then-retried enrichment case the
-- audit found). Idempotent via the partial unique index.
DROP TRIGGER IF EXISTS trg_lead_quotes_referral_materialize_upd ON public.lead_quotes;
CREATE TRIGGER trg_lead_quotes_referral_materialize_upd
  AFTER UPDATE OF enrichment_status, quote_summary ON public.lead_quotes
  FOR EACH ROW
  WHEN (NEW.enrichment_status = 'enriched')
  EXECUTE FUNCTION public.trg_lead_quotes_referral_materialize();
