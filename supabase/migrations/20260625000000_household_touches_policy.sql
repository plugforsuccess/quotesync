-- Touch history on the customer page didn't say WHICH policy each touch was for —
-- a household with an AUTO and a RENTERS renewal showed two identical "Renewal
-- call" rows. The attempt already joins to its specific case, so the policy is
-- known; surface product + policy_no from each source case (renewal/cancel; null
-- for lead/cross-sell/service which aren't single-policy). Return signature
...
RETURNS TABLE(touched_at timestamptz, source text, method text, result text, note text, employee_name text, product text, policy_no text)
